...
public class BasicServer {
  ...
  private static class DatagramServer implements Runnable {
    ...
    public void run() {
      ...
      while (true) {
        ...
        try {
          ...
          ResponseHandler rh =
              createResponseHandler(dgramSocket, dgramPacket.getAddress(), dgramPacket.getPort());
    ...
    private ResponseHandler createResponseHandler(
        final DatagramSocket dgramSocket, final InetAddress destAddress, final int destPort) {
    ...
    public void run() {
      try {
        ...
        while (!clientSocket.isClosed()) {
          ...
          System.out.println(String.format("Received new message: %d bytes", msgBuffer.length));
